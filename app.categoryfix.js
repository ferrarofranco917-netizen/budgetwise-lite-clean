...
csvMappingFieldsTitle: '🎯 Field mapping:',
    ...
    applyLanguage() {
        ...
        this.updateChart();
        // Aggiorna label categorie (anche nel select e nel manager) in base alla lingua
        if (typeof this.updateAllCategorySelects === 'function') {
            this.updateAllCategorySelects();
        }
        if (document.getElementById('categoryManagerOverlay') && document.getElementById('categoryManagerOverlay').style.display === 'flex') {
    ...
    refreshCategoryList() {
        ...
        if (defaultList) {
            defaultList.innerHTML = this.defaultCategories.map(cat => 
                `<div class="category-item default"><span>${this.getCategoryEmoji(cat)} ${this.getCategoryDisplayName(cat)}</span></div>`
            ).join('');
        }
    ...
    updateAllCategorySelects() {
        ...
        const optionsHtml = categories.map(cat => 
            `<option value="${cat}">${this.getCategoryEmoji(cat)} ${this.getCategoryDisplayName(cat)}</option>`
        ).join('');
    ...
    getCategoryDisplayName(category) {
        const cat = String(category || 'Altro');
        const lang = this.data.language || 'it';
        // Default categories are stored in Italian to keep data stable in localStorage.
        ...
        return (map[lang] && map[lang][cat]) ? map[lang][cat] : cat;
    }
