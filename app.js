...
csvMappingFieldsTitle: '🎯 Field mapping:'            },
    ...
    applyLanguage() {
        ...
        // ==================== CATEGORY MANAGEMENT (i18n) ====================
        const manageCategoriesBtn = document.getElementById('manageCategoriesBtn');
        if (manageCategoriesBtn) {
            manageCategoriesBtn.textContent = this.t('manageCustomCategories');
            const categoriesCard = manageCategoriesBtn.closest('.section-card');
            const categoriesH2 = categoriesCard ? categoriesCard.querySelector('h2') : null;
            if (categoriesH2) categoriesH2.textContent = this.t('categoriesSectionTitle');
        }

        const categoryOverlay = document.getElementById('categoryManagerOverlay');
        if (categoryOverlay) {
            const titleH3 = categoryOverlay.querySelector('h3');
            if (titleH3) titleH3.textContent = this.t('categoriesSectionTitle');

            const h4s = categoryOverlay.querySelectorAll('h4');
            if (h4s && h4s.length) {
                if (h4s[0]) h4s[0].textContent = this.t('defaultCategoriesTitle');
                if (h4s[1]) h4s[1].textContent = this.t('yourCategoriesTitle');
            }

            const newCatLabel = categoryOverlay.querySelector('label[for="newCategoryName"]');
            if (newCatLabel) newCatLabel.textContent = this.t('newCategoryLabel');

            const newCatInput = document.getElementById('newCategoryName');
            if (newCatInput) newCatInput.placeholder = this.t('newCategoryPlaceholder');

            const closeBtn = document.getElementById('closeCategoryManager');
        ...
        container.innerHTML = expenses.map(exp => {
            ...
            return `
                <div class="expense-item">
                    <div class="expense-info">
                        <span class="expense-name">${exp.name || '?'}</span>
                        <span class="expense-category">${this.getCategoryLabel(catDisplay)}</span>
                    </div>
                    <span class="expense-amount">${this.formatCurrency(exp.amount || 0)}</span>
                    <div class="expense-actions">
    ...
    refreshCategoryList() {
        ...
        if (defaultList) {
            defaultList.innerHTML = this.defaultCategories.map(cat => 
                `<div class="category-item default"><span>${this.getCategoryLabel(cat)}</span></div>`
            ).join('');
        }
...
getCategoryLabel(category) {
    ...
    if (key) return this.t(key); // include emoji
    return `📌 ${category}`;
}
...
getCategoryEmoji(category) {
    ...
    setupAiActions() {
        ...
        // Evita doppi listener se la funzione viene richiamata
        applyBtn.onclick = null;
        dismissBtn.onclick = null;

        applyBtn.addEventListener('click', (e) => {
            const type = e.currentTarget.dataset.type || '';
            const amount = parseFloat(e.currentTarget.dataset.amount || '0') || 0;

            const bumpGoal = (delta) => {
                const currentGoal = parseFloat(this.data.savingsGoal || 0) || 0;
                const nextGoal = Math.max(0, currentGoal + (delta || 0));
                this.data.savingsGoal = nextGoal;
                const goalInput = document.getElementById('saveGoal');
                if (goalInput) goalInput.value = nextGoal;
                this.saveData();
                this.updateUI();
                this.showToast(
                    this.data.language === 'it'
                        ? `🎯 Obiettivo aumentato a ${this.formatCurrency(nextGoal)}`
                        : `🎯 Goal increased to ${this.formatCurrency(nextGoal)}`,
                    'success'
                );
            };
            ...
            const actionEl = document.getElementById('aiAction');
            const widgetEl = document.getElementById('aiWidget');
